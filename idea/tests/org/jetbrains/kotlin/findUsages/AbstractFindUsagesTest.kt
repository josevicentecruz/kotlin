...
//        }
        ...
        try {
            ...
            if (isLibraryElement) {
                ...
                findUsagesAndCheckResults(mainFileText, prefixForCheck, rootPath, originalElement, options, project, isFirPlugin)

                val navigationElement = caretElement.navigationElement
                if (navigationElement !== originalElement) {
                    findUsagesAndCheckResults(mainFileText, prefixForCheck, rootPath, navigationElement, options, project, isFirPlugin)
                }
            } else {
                findUsagesAndCheckResults(mainFileText, prefixForCheck, rootPath, caretElement, options, project, isFirPlugin)
            }
        } finally {
...
internal fun <T : PsiElement> findUsagesAndCheckResults(
    ...
    if (log != null) {
        ...
        try {
            ...
            findUsagesAndCheckResults(mainFileText, prefix, rootPath, caretElement, options, project, isFirPlugin)
        } finally {
            ExpressionsOfTypeProcessor.mode = ExpressionsOfTypeProcessor.Mode.ALWAYS_SMART
        }
    }
